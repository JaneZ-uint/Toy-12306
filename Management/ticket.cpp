...
void TicketSystem::buy_ticket(JaneZ::String<22> &username, JaneZ::String<22> &trainID, JaneZ::Date &day, int num, JaneZ::String<42> &startStation, JaneZ::String<42> &endStation, bool waitList, TrainSystem &train_system, UserSystem &user_system,int timeStamp) {
    ...
    JaneZ::Date earliestLeave = JaneZ::Date::addMinutesToDateTime(currentTrain.saleStartDate,currentTrain.startTime,currentTrain.stations[i].travelTime + currentTrain.stations[i].stopoverTime);//出发日期必须算上停留时间
    JaneZ::Date latestLeave = JaneZ::Date::addMinutesToDateTime(currentTrain.saleEndDate,currentTrain.startTime,currentTrain.stations[i].travelTime + currentTrain.stations[i].stopoverTime);
