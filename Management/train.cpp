...
bool TrainSystem::add_train(JaneZ::String<22> &trainID, int stationNum, int seatNum,
                            sjtu::vector<JaneZ::String<42> > &stations, sjtu::vector<int> &prices,
                            JaneZ::Clock &startTime, sjtu::vector<int> &travelTimes, sjtu::vector<int> &stopoverTimes,
                            JaneZ::Date &saleStartDate, JaneZ::Date &saleEndDate, char type) {
    ...
    for (int i = 0; i < stationNum; i++) {
        ...
        if (i == 0) {
            ...
            current.stations[i].travelTime = current.stations[i - 1].travelTime + current.stations[i - 1].stopoverTime +
                                             travelTimes[i - 1];
            current.stations[i].stopoverTime = 0;
        } else {
            current.stations[i].price = current.stations[i - 1].price + prices[i - 1];
...
void TrainSystem::query_ticket(JaneZ::String<42> &s, JaneZ::String<42> &t, JaneZ::Date &d, JaneZ::SortType SortWay) {
    ...
    for (int i = 0; i < possibleSolution.size(); i++) {
        JaneZ::Date earliestDate = JaneZ::Date::addMinutesToDateTime(possibleSolution[i].st.saleBeginDate,
                                                                     possibleSolution[i].st.StartClock,
                                                                     possibleSolution[i].st.leavingTimeCost);
        JaneZ::Date latestDate = JaneZ::Date::addMinutesToDateTime(possibleSolution[i].st.saleEndDate,
                                                                   possibleSolution[i].st.StartClock,
                                                                   possibleSolution[i].st.leavingTimeCost);
    ...
    if (SortWay == JaneZ::SortType::time) {
        ...
        while (!timePQ.empty()) {
            ...
            JaneZ::Date earliestDate = JaneZ::Date::addMinutesToDateTime(
                topValue1.saleBeginDate, topValue1.StartClock, topValue1.leavingTimeCost);
...
void TrainSystem::query_transfer(JaneZ::String<42> &s, JaneZ::String<42> &t, JaneZ::Date &d, JaneZ::SortType SortWay) {
    ...
    for (int i = 0; i < Start.size(); i++) {
        ...
        JaneZ::Date earliestDate = JaneZ::Date::addMinutesToDateTime(firstTrain.saleStartDate, firstTrain.startTime,
                                                                     Departure.travelTime + Departure.stopoverTime);
        JaneZ::Date latestDate = JaneZ::Date::addMinutesToDateTime(firstTrain.saleEndDate, firstTrain.startTime,
                                                                   Departure.travelTime + Departure.stopoverTime);
        if (d < earliestDate || d > latestDate) {
            continue;
        }
        ...
        for (int j = 0; j < To.size(); j++) {
            ...
            for (int k = 0; k < terminalNum; k++) {
                //遍历第二辆车第0站到终点站前一站
                for (int l = departNum + 1; l < firstTrain.stationNum; l++) {
                    //遍历第一辆车从起点站到最后一战
                    ...
                    if (midArrive > secondLatest) {
                        ...
                        if (firstClock <= secondClock) {
                            ...
                            //隔夜换乘
                            int waitTime = secondClock.hour * 60 + secondClock.minute + 24 * 60 - firstClock.hour * 60 -
                                           firstClock.minute;
                            int totalTime = firstRunningTime + secondRunningTime + waitTime;
                            current.totalTime = totalTime;
